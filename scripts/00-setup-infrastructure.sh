...
echo "     - Automatic SSL certificate management (Certbot)"
...
###############################################################################
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Step 2: Nginx Reverse Proxy${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""

if [ -f "$SCRIPT_DIR/03-nginx-setup.sh" ]; then
    bash "$SCRIPT_DIR/03-nginx-setup.sh"
else
    echo -e "${RED}Error: Nginx setup script not found${NC}"
    echo "Expected: $SCRIPT_DIR/03-nginx-setup.sh"
    exit 1
fi
...
echo ""
echo "   Example DNS records (Namecheap):"
echo "   - A Record: app  → YOUR_VPS_IP"
echo "   - A Record: api  → YOUR_VPS_IP"
echo ""

echo -e "${YELLOW}2. Obtain SSL Certificates${NC}"
echo ""
echo "   After DNS is configured, run:"
echo "   ${BLUE}./scripts/04-ssl-setup.sh${NC}"
echo ""
echo "   This will:"
echo "   - Request SSL certificates from Let's Encrypt"
echo "   - Guide you through enabling HTTPS"
echo ""
